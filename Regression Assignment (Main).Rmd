---
title: "Regression Assignment (dduyi)"
...
dffits_fullmodel<-dffits(fullmodel)
plot(dffits_fullmodel)
abline(h= 2 * sqrt((length(fullmodel$coefficients)) / nrow(Dataset)),col="red")
dffits_threshold=2*(sqrt((length(fullmodel$coefficients))) / nrow(Dataset))
...
high_dffits_points<-which(abs(dffits_fullmodel)>dffits_threshold)
high_cooks_distance_points<-which(cd_fullmodel>cd_threshold)

all_potential_issues=union(high_studentized_residual_points,high_leverage_points,high_dffits_points,high_cooks_distance_points)
